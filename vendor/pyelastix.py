...
def _find_executables(name):
    ...
    if sys.platform.startswith('win'):
        ...
        possible_locations.extend(['/usr/bin','/usr/local/bin'])
...
#                   '-out', tempdir, '-p', path_params]
    ...
    if True:
        ...
        command = str(get_elastix_exes()[1])+' -def all -out '+str(tempdir)+' -tp '+str(path_trafo_params)
        _system3(command, verbose)
    ...
    with open(path_trafo_params, 'r') as searchfile:
        for line in searchfile:
            if 'TransformParameters' in line:
                transform_parameters = np.fromstring(line[21:-1], sep=' ')
                break
        
    # Clean and return
    _clear_temp_dir()
    return a, fields, transform_parameters
